...
class RevenueHistoryProjection(models.TransientModel):
    ...
    def get_revenue_history_projection(self):
        ...
        if self.type_report == "income":
            invoice_type = ('move_type', 'in', ('out_invoice', 'out_refund'))
        else:
            invoice_type = ('move_type', 'in', ('in_invoice', 'in_refund'))
        # validation report_type
        for w in week_colums:
            end = start + timedelta(days=6)
            week = start.isocalendar()[1]
            if w == 1:
                invoice_ids = account_move_obj.search(
                    [('payment_state', '=', 'not_paid'), ('invoice_date_due', '>=', today), ('invoice_date_due', '<=', end), invoice_type])
                date_range_from_week = '%s - %s' % (datetime.now().strftime('%d/%m/%Y'),
                                                    end.strftime('%d/%m/%Y'))
            elif w == 8:
                invoice_ids = account_move_obj.search(
                    [('payment_state', '=', 'not_paid'), ('invoice_date_due', '>=', start), invoice_type])
                date_range_from_week = ' Del %s en delante ' % start.strftime(
                    '%d/%m/%Y')
            else:
                invoice_ids = account_move_obj.search(
                    [('payment_state', '=', 'not_paid'), ('invoice_date_due', '>=', start), ('invoice_date_due', '<=', end), invoice_type])
                date_range_from_week = '%s - %s' % (
                    start.strftime('%d/%m/%Y'), end.strftime('%d/%m/%Y'))
            total = 0.0

            for i in invoice_ids:
                invoice_residual = i.amount_residual
                if i.move_type == 'out_refund':
...
class RevenueHistoryProjectionLine(models.TransientModel):
    ...
    residual = fields.Float('Residual', digits=(8, 2))
